...
class CruisePower(Component):
    ...
    def __init__(self):
        ...
        self.add_param('Vehicle', val=u'helicopter', description='Vehicle type',
                       pass_by_obj=True)  # 'tiltwing' or 'helicopter'
        self.add_param('rProp', val=1.4, description='prop/rotor radius [m]')
        self.add_param('V', val=50.0, description='Cruise speed [m/s]')
        self.add_param('W', val=2000.0, description='Weight of vehicle [N]')
        ...
        self.add_output('bRef', val=0.0,
                        description='reference wingspan assuming 2 props per wing with outboard props are at wingtips, 1 meter wide fuselage plus clearance between props and fuselage')
        ...
        self.add_output('Ct', val=0.0,
                        description='Thrust coefficient (including tip loss factor for effective disk area)')
    ...
    def solve_nonlinear(self, params, unknowns, resids):
        ...
        if (params['Vehicle'].lower().replace('-', '') == "tiltwing"):
            ...
            unknowns['Ct'] = params['W'] / (
                        rho * math.pi * params['rProp'] ** 2 * unknowns['B'] ** 2 * unknowns['omega'] ** 2 * params[
                    'rProp'] ** 2)
            ...
            for i in range(5):
                unknowns['lambda'] = (unknowns['mu'] * math.tan(unknowns['alpha']) + \
                                     ...
                                     (1.0 + unknowns['Ct'] / 2.0 * unknowns['lambda'] / (
                                                 unknowns['mu'] ** 2 + unknowns['lambda'] ** 2.0) ** 1.5)
            unknowns['v'] = unknowns['lambda'] * unknowns['omega'] * params['rProp'] - params['V'] * math.sin(
                unknowns['alpha'])

            # Power in forward flight (see "Helicopter Theory" section 5-12)
            unknowns['PCruise'] = params['W'] * (
                        params['V'] * math.sin(unknowns['alpha']) + 1.3 * math.cosh(8 * unknowns['mu'] ** 2) * unknowns[
                    'v'] + \
